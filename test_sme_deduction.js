...
function getSmeRate(cd, revenue, region) {
  ...
  if (isSmall) {
    if (region === 'dense') {
      rate = isRetailMedical ? 5 : 10;   // 소기업 과밀억제권역
    } else if (region === 'capital_non_dense') {
      rate = isRetailMedical ? 10 : 20;  // 소기업 수도권(과밀억제 외)
    } else {
...
assert('소기업+제조+과밀억제권역 = 10%', r.rate, 10);
...
assert('소기업+도소매+과밀억제권역 = 5%', r.rate, 5);
...
assert('소기업+보건+과밀억제권역 = 5%', r.rate, 5);
